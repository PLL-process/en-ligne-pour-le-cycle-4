...
#: « Activité 3 » vit parfois dans un <span class="num"> voisin du titre, pas dedans.
NUMERO = re.compile(r'<span[^>]*class="[^"]*\bnum\b[^"]*"[^>]*>(.*?)</span>', re.S)


def sections(texte):
    """Découpe la page en sections : (titre, début du contenu, fin du contenu).

    Le titre rendu inclut le numéro d'activité même quand la page l'écrit à côté
    du <h_> plutôt que dedans — plusieurs lots du Thème 2 font ainsi :
    `<header><span class="num">Activité 1</span><h3>Le facteur du réseau</h3></header>`.
    Lire le seul <h3> ferait manquer six séquences sur dix-sept.
    """
    titres = list(re.finditer(r"<h([234])[^>]*>(.*?)</h\1>", texte, re.S))
    for k, m in enumerate(titres):
        fin = titres[k + 1].start() if k + 1 < len(titres) else len(texte)
        libelle = html.unescape(re.sub(r"<[^>]+>", " ", m.group(2)))
        voisins = NUMERO.findall(texte[max(0, m.start() - 260):m.start()])
        if voisins:
            libelle = html.unescape(re.sub(r"<[^>]+>", " ", voisins[-1])) + " — " + libelle
        yield " ".join(libelle.split()), m.end(), fin


#: Un titre d'activité S'ANNONCE : il COMMENCE par « Activité 3 », « Séance 2 »…
#: La première version cherchait ces mots N'IMPORTE OÙ dans le titre. Sur le
#: Thème 1 cela passait ; sur le Thème 2, où les lots s'appellent « atelier »,
#: quatre séquences sur six recevaient le bloc sous « 📚 Le référentiel de
#: l'ATELIER » — une section qui décrit la séquence, et où personne ne travaille.
#: C'est la règle n°184 une fois de plus : je reconnaissais un mot, pas une
#: situation. Un test ancré au début du titre, plus une liste de sections qui ne
#: sont jamais des activités, suffit à ne plus se tromper.
ACTIVITE = re.compile(r"^[\W\d]{0,6}(activit[ée]|séance|seance|étape|etape|tp|manipulation)\b", re.I)

#: sections qui PARLENT du travail sans en être : on n'y pose jamais de rôles
META = re.compile(r"référentiel|referentiel|trois façons|trois facons|synthèse|synthese"
                  r"|bilan|problèmes? intermédiaires?|mission|situation|différenciation"
                  r"|differenciation|évaluation|evaluation|bonus|billet d'entrée"
                  r"|ce que tu as déjà|prêt·e|contrat", re.I)
