/* Contrôle d'une séquence C7.7 — l'atelier, les activités, la mémoire.
 *
 * Usage : node test77.mjs <sequence.html> <niveau> <attendus.json>
 *
 * Ce que ce script vérifie que l'œil ne vérifie pas :
 *   · qu'AUCUN verrou expérientiel n'est ouvert à l'ouverture de la page
 *     (règle d'or n°226) ;
 *   · que la colonne d'état des cotes affiche exactement ce que la séquence
 *     annonce — « trois lignes portent une croix » doit être vrai ligne par
 *     ligne, et pas seulement en nombre ;
 *   · qu'une correction PARTIELLE ne rend aucun moyen capable. C'est la
 *     démonstration de la séance de 3e, et un test qui ne regarderait que
 *     l'état final la manquerait entièrement (règle d'or n°225) ;
 *   · que les cotes modifiées sans nécessité sont comptées à part ;
 *   · qu'une seule instance de l'atelier existe dans la page.
 */
...
ok('un seul atelier dans la page', (await p.$$eval('#etabli', n => n.length)) === 1);
...
if (REP.editable) {
  const cotes = await p.$$eval('table.traits .val input', n => n.map(e => e.value));
  ok('les cotes sont écrites à la française', cotes.every(v => !v.includes('.')), cotes.join(' '));
}

// ── l'atelier ────────────────────────────────────────────────────────────
const etat = () => p.evaluate(() => ({
  nRet: document.getElementById('nRet').textContent,
  j2: (document.getElementById('nDelai') || document.getElementById('nInutile')).textContent,
  capables: [...document.querySelectorAll('#tab tr.ok')].map(tr => tr.cells[0].textContent),
  etats: [...document.querySelectorAll('table.traits tr[id^=li_]')]
    .map(tr => tr.id.slice(3) + ':' + tr.cells[0].textContent),
...
const regler = async r => {
  for (const [id, v] of Object.entries(r || {})) {
    ...
    if (tag === 'SELECT') await p.selectOption('#' + id, String(v));
    else await p.fill('#' + id, String(v));
  }
  await p.waitForTimeout(100);
};

for (const e of REP.atelier.essais) {
  await p.click('#reinit'); await p.waitForTimeout(120);
  await regler(e.regle);
  await p.click('#evaluer'); await p.waitForTimeout(150);
  const s = await etat();
  ok('« ' + e.titre + ' » : ' + e.nRet + ' moyen(s) capable(s)', s.nRet === e.nRet, 'lu ' + s.nRet);
  if (e.j2 !== undefined)
    ok('« ' + e.titre + ' » : seconde jauge à ' + e.j2, s.j2 === e.j2, 'lu ' + s.j2);
  if (e.capables)
    ok('« ' + e.titre + ' » : ce sont les bons',
      e.capables.every(x => s.capables.some(y => y.includes(x))) && s.capables.length === e.capables.length,
      s.capables.join(' · ').slice(0, 80));
  if (e.etats)
    ok('« ' + e.titre + ' » : la colonne d’état dit exactement cela',
      s.etats.join(' ') === e.etats.join(' '), s.etats.join(' '));
  if (e.verdict)
    ok('« ' + e.titre + ' » : le verdict le dit', s.dernier.includes(e.verdict),
      s.dernier.slice(0, 80));
  if (e.exp)
    ok('« ' + e.titre + ' » : le verrou ' + e.exp + ' s’ouvre',
...
ok('le code d’appui est cité', src.includes(REP.appui), REP.appui);
...
ok('l’état de l’atelier survit au rechargement',
  (await p.inputValue('#' + REP.temoin[0])) === REP.temoin[1],
  REP.temoin[0] + ' = ' + (await p.inputValue('#' + REP.temoin[0])));
ok('les verrous survivent au rechargement',
  await p.evaluate(v => !!window.__exp[v], REP.atelier.essais[0].exp || 'evalue'));
