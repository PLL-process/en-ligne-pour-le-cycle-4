# Fiche pédagogique / inspection — 4e_C7 (LOT 01)

## Identification
- **Niveau** : 4e
- **Codes** : 4e_C7.1 · 4e_C7.2 · 4e_C7.3
- **Thème** : 3 — Création, conception, réalisation, innovations
- **Programme** : Technologie Cycle 4 (BO n°9 du 29 février 2024)
- **Durée** : 4 × 55 min

## Compétences et connaissances
- **C7** : Imaginer, concevoir et réaliser une ou des solutions…
- **Sous-compétences** :
  - 4e_C7.1 Organiser un processus de conception et de réalisation…
  - 4e_C7.2 Proposer et fabriquer une solution pour ajouter une nouvelle fonction…
  - 4e_C7.3 Comparer différents matériaux pour choisir le plus adapté.
- **Domaines du socle** : D2, D3, D4, D5
- **CRCN** : Création de contenu / Résoudre des problèmes techniques

## Prérequis
- Notions de base sur les objets techniques (Thème 1/2)
- Expérience préalable possible avec le « Jardin connecté » (4e_C6.2)

## Situation déclenchante
Le collège souhaite améliorer son jardin pédagogique. Les élèves proposent et réalisent une amélioration concrète (support, abri, indicateur) en tenant compte du climat martiniquais.

## Problématique
Comment imaginer, choisir et réaliser une solution technique simple pour améliorer l’arrosage ou le suivi d’un espace vert du collège, en tenant compte des contraintes locales ?

## Déroulé séance par séance
1. **Séance 1** : Situation + hypothèses + Activité 1 (planning)
2. **Séance 2** : Activité 2 (propositions de solutions)
3. **Séance 3** : Activité 3 (comparaison de matériaux) + choix
4. **Séance 4** : Réinvestissement + synthèse + bilan + QCM

## Matériel
- Version A : matériaux récup (bois palette, boîtes plastiques, carton), outils de base, éventuellement capteurs déjà connus
- Version B : simulation / croquis numériques (draw.io)
- Version C : documents + tableaux papier + photos

## Organisation
Travail individuel puis binômes pour la comparaison. Mise en commun des justifications.

## Différenciation & inclusion
Aides progressives (niveau 1 / 2), tableaux pré-structurés, possibilité de dictée à un pair, supports visuels.

## Évaluation
- Formative : productions des activités
- Entraînement : QCM séparé
- Critères LSU : organisation du travail, argumentation, prise en compte des contraintes

## Bilan et prolongements
Lien naturel vers la réalisation (C7.5+) et vers la programmation (C9 / 3e).

---
Document produit dans le cadre du projet « En ligne pour le cycle 4 » — Agent Grok (Thème 3)
