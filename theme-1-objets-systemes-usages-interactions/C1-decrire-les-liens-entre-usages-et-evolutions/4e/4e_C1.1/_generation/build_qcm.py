# -*- coding: utf-8 -*-
"""Assemble le QCM 4e_C1.1 à C1.3 à partir du gabarit maison et de `q.py`.
...
JALONS = {
    0: "/* ═══ C1.1 — Mettre en relation les OST avec leurs usages ═══ */",
    12: "/* ═══ C1.2 — Avantages et inconvénients des évolutions ═══ */",
    21: "/* ═══ C1.3 — Justifier l'évolution d'un OST ═══ */",
}
...
def construire(gabarit: str, sortie: str) -> None:
    ...
    remplacements = [
        ("/* Banque de questions — Shenzhen 3e : 30 questions "
         ...
         "/* Banque de questions — Tsinghua 4e : 30 questions sur les trois codes "
         "(C1.1 : 12 · C1.2 : 9 · C1.3 : 9), dont 6 illustrées. Chiffres publics et "
         "sourcés : ministère de l'Intérieur, JRC/EFFIS, ADEME. */"),

        ('const KEY="qcm_3e_C3.1-C3.4_shenzhen";', 'const KEY="qcm_4e_C1_tsinghua";'),
  ...
  "C3.2":"3e_C3.2 — Choisir et argumenter en tenant compte du cycle de vie et des trois piliers" };""",
         ...
         """const COMP_LABELS={
  "C1.1":"4e_C1.1 — Mettre en relation les OST avec leurs usages.",
  "C1.2":"4e_C1.2 — Identifier les avantages et les inconvénients associés aux évolutions technologiques et informatiques.",
  "C1.3":"4e_C1.3 — Justifier l\u2019évolution d\u2019un OST pour répondre à l\u2019évolution des besoins." };"""),

        ("<title>Thème 1 · 3e — QCM : Shenzhen, comment refroidir un local qui surchauffe ? "
         "(3e_C3.1 à C3.4)</title>",
         "<title>Thème 1 · 4e — QCM : Tsinghua, concevoir avant de connecter "
         "(4e_C1.1 à C1.3)</title>"),
        ...
        ("<h1>Thème 1 · 3e — QCM : Shenzhen, comment refroidir un local qui surchauffe ?</h1>",
         "<h1>Thème 1 · 4e — QCM : Tsinghua, concevoir avant de connecter</h1>"),
        ('<p class="subtitle">Caractériser et choisir une solution (3e_C3.1 à C3.4) — protocole '
         ...
         '<p class="subtitle">Des données publiques, une estimation qu\'on ose faire et dont on '
         "ose dire les limites — mettre en relation les OST avec leurs usages (4e_C1.1), "
         "peser les évolutions (C1.2), justifier par le besoin (C1.3)</p>"),
        ...
        ('<span class="badge code">3e_C3.1</span><span class="badge code">3e_C3.2</span>'
         ...
         '<span class="badge code">4e_C1.1</span><span class="badge code">4e_C1.2</span>'
         '<span class="badge code">4e_C1.3</span>'),

        ("30 questions · 1 illustrée · chaque distracteur réfuté",
         "30 questions · 6 illustrées · chaque distracteur réfuté"),
      ...
      <option value="C3.2">3e_C3.2 — Choisir et argumenter sur les trois piliers</option>""",
         """      <option value="C1.1">4e_C1.1 — Mettre en relation les OST avec leurs usages</option>
      <option value="C1.2">4e_C1.2 — Avantages et inconvénients des évolutions</option>
      <option value="C1.3">4e_C1.3 — Justifier l'évolution par le besoin</option>"""),

        ("QCM d’entraînement 3e_C3.1 à C3.4 · Thème 1",
         "QCM d’entraînement 4e_C1.1 à C1.3 · Thème 1"),
    ]
    ...
    s = s.replace("sequence_3e_C3.1-C3.4_shenzhen.html",
                  "sequence_4e_C1.1-C1.3_tsinghua_feux.html")

    # zone d'accueil de la règle n°45, juste avant le lien de retour
    ancre = '<p style="text-align:center"><a href="sequence_4e_C1.1-C1.3_tsinghua_feux.html">'
    if s.count(ancre) != 1:
...
n'est pas dans le dépôt. Or le fichier qcm_4e_C1.1-C1.3_tsinghua.html (33 questions) a été
modifié directement depuis — les campagnes du thème 1 (anti-longueur 28/08, boîtes modales 29/08, impression 02/09, n°303 21/09) et les trois questions du 24/09. Relancer ce script
effacerait ces modifications sans le dire.

La source de vérité est désormais qcm_4e_C1.1-C1.3_tsinghua.html lui-même : modifie-le à
